#===================================================
## main code to train and evalute the model
#===================================================
...
import pickle
from pgrnet.pgr import pgrnet
from pgrnet import JointsCombinedLoss
...
def main(args):
    ...
    print(full[0].shape)
    full[0] = full[0][:, :, :, :, 0]
    datamode = "T1" if args.modality=="t1" else "T2"
    ## Create a dataset loader
    full_dataset_train = image_Dataset(image_paths=full[0][0:train_idx], target_paths=full[1][:train_idx],mode=datamode)
    full_dataset_val = image_Dataset(image_paths=full[0][train_idx:validation_idx],
                                        target_paths=full[1][train_idx:validation_idx], use_flip = False,mode=datamode)
    ...
    # create model
    print("==> creating PGR-Net  model")
    model = pgrnet(num_stacks=args.stacks, num_blocks=args.blocks, num_classes=args.njoints)
    model = torch.nn.DataParallel(model).to(device)

    # define loss function (criterion) and optimizer

    criterion = JointsCombinedLoss().to(device)
    ...
    if args.resume:
       ...
       model.load_state_dict(torch.load(f'../weights/model_{args.modality}_pgr_{args.stacks}', map_location='cpu')['model_weights'])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if valid_acc > best_acc:
           ...
           torch.save(state, f'../weights/model_{args.modality}_pgr_{args.stacks}')
           best_acc = valid_acc
...
def train(train_loader, model, criterion, optimizer):
    ...
    for i, (input, target, vis, skeleton) in enumerate(train_loader):
        # measure data loading time
        data_time.update(time.time() - end)
        input, target, skeleton = input.to(device), target.to(device, non_blocking=True), skeleton.to(device)
        vis = vis.to(device, non_blocking=True)
        # compute output
        output = model(input, skeleton) 
...
def validate(val_loader, model, criterion):
    ...
    with torch.no_grad():
        for i, (input, target, vis, skeleton) in enumerate(val_loader):
            # measure data loading time
            data_time.update(time.time() - end)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Verterbal disc labeling PGR-Net approach')
