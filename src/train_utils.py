# modified and adapted by Alharbi
# Author: Lucas
...
class image_Dataset(Dataset):
    def __init__(self, image_paths, target_paths, use_flip = True, mode="T1"):  # initial logic happens like transform
        ...
        if mode=="T2":
            self.skelet = np.load('../prepared_data/t1_Skelet_gaussian_heatmap.npy')  # load the average skeleton
        else:
            self.skelet = np.load('../prepared_data/t1_Skelet_gaussian_heatmap.npy')  # load the average skeleton
    @staticmethod
    def rotate_img(img):
    ...
    def __getitem__(self, index):
        ...
        return t_image, t_mask, vis, np.expand_dims(self.skelet,axis=0)
